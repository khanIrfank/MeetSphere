...
export default function HowItWorks() {
  return (
    <section className="relative py-24 sm:py-32 border-y border-soft select-none overflow-visible">
      ...
      {/* Sticky Stacking Cards Container (MWToken Vision Style) */}
      <div className="max-w-7xl mx-auto px-5 sm:px-8 relative space-y-12 pb-24">
        {stackingCards.map((card) => (
          ...
          >
            <div className={`rounded-[2.5rem] border ${card.borderColor} ${card.bgColor} p-6 sm:p-10 shadow-2xl shadow-black/50 backdrop-blur-xl overflow-hidden grid lg:grid-cols-12 gap-8 items-center`}>
              ...
              <div className="lg:col-span-6">
                <div className="relative rounded-2xl overflow-hidden aspect-[4/3] border border-white/10 shadow-xl group">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between bg-black/70 backdrop-blur-md p-3.5 rounded-xl border border-white/15">
                    <div className="flex items-center gap-2">
                      <span className="h-2.5 w-2.5 rounded-full bg-brand-400 animate-ping" />
                      <span className="text-xs font-extrabold text-white">STEP {card.step} VISION STREAM</span>
                    </div>
                    <span className="text-[10px] font-mono font-bold text-brand-400 bg-brand-500/20 px-2.5 py-1 rounded-full border border-brand-500/30">
                      ACTIVE
                    </span>
                  </div>
                </div>
