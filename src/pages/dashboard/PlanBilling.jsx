...
export default function PlanBilling() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-8 py-8 space-y-8 select-none">
      ...
      {/* Active Subscriptions Summary Banner (Renders ALL owned active plans with Deactivate options!) */}
              ...
              <div key={plan.id} className="bg-surface/80 border border-brand-500/40 p-4 rounded-2xl shadow-sm space-y-3 relative flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-[11px] text-theme-sub font-semibold mt-0.5">
                    Status: <span className="text-emerald-400 font-bold">Active</span> ({isYearly ? plan.yearlyPriceDisplay + '/yr' : plan.priceDisplay + '/mo'})
                  </p>
                </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {ROOM_PLANS.map((p) => {
            ...
            const price = isYearly ? p.yearlyPriceDisplay : p.priceDisplay
            const period = isYearly ? p.yearlyPeriod : p.period
            ...
            return (
              ...
              >
                ...
                <div>
                  <h4 className="font-extrabold text-lg text-theme-heading">{p.name}</h4>
                  <p className="text-xs text-theme-sub font-semibold mt-1 mb-4 h-10">{p.tagline}</p>

                  <div className="mb-4">
                    <div className="flex items-baseline gap-1">
                      <span className="font-display text-3xl font-extrabold text-theme-heading">{price}</span>
                      <span className="text-xs text-theme-sub font-semibold">{period}</span>
                    </div>
                    {isYearly && (
                  ...
                  {/* Room Limits Pill */}
                  <div className="bg-surface/70 border border-soft rounded-xl p-2.5 mb-5 space-y-1 text-xs">
                  ...
                  <ul className="space-y-2.5 mb-6">
                    {p.features.map((f) => (
                      <li key={f} className="flex items-start gap-2 text-xs text-theme-body font-medium">
                ...
                {isSubscribed ? (
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-rose-500/15 hover:bg-rose-500 text-rose-400 hover:text-white font-extrabold text-xs border border-rose-500/30 transition-all flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                  >
                    <Power size={13} />
                    <span>Deactivate Plan</span>
                  </button>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-brand-500 hover:bg-brand-400 text-ink-950 font-extrabold text-xs shadow-md transition-all hover:scale-105 active:scale-95 cursor-pointer"
                  >
                    Subscribe to {p.name}
                  </button>
                )}
