...
import { Plus, Trash2, Copy, CalendarClock, History, Calendar, Zap, AlertCircle, ArrowRight } from 'lucide-react'
...
export default function MeetingsList() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-8 py-6 space-y-6 select-none">
      {/* Missing Plan Alert Banner */}
      {!hasActivePlan && (
        <div className="rounded-2xl bg-amber-500/10 border border-amber-500/30 p-3.5 flex items-center justify-between gap-3 text-xs shadow-sm">
          <div className="flex items-center gap-2.5">
            <AlertCircle size={18} className="text-amber-400 shrink-0" />
            <div>
              <p className="font-extrabold text-amber-300">No Active Room Plan</p>
              <p className="text-[11px] text-slate-300 font-medium">Please subscribe to a Room Plan to schedule or start meetings.</p>
            </div>
          </div>
          <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            {hasActivePlan && (
              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-brand-500/15 text-brand-600 dark:text-brand-400 text-[10px] font-extrabold border border-brand-500/30">
                <Zap size={11} /> {activePlan.name} ({activePlan.maxUsers} Users)
              </span>
            )}
          </div>
          <p className="text-xs sm:text-sm font-semibold text-theme-sub mt-1">
            Manage scheduled meetings and view past meeting history.
          </p>
        </div>
        <Button icon={Plus} size="sm" onClick={handleScheduleClick}>Schedule Meeting</Button>
      </div>
      ...
      {activeTab === 'scheduled' && (
        <div className="max-h-[440px] overflow-y-auto scrollbar-custom pr-1.5">
          {meetings.length === 0 ? (
            <div className="rounded-2xl border border-soft bg-elevated py-16 text-center">
              ...
              <p className="text-sm font-semibold text-theme-sub">No scheduled meetings yet. Click Schedule Meeting to add one.</p>
            </div>
          ) : (
            <ul className="flex flex-col gap-3">
              {meetings.map((m) => (
                <li key={m.id} className="rounded-2xl border border-soft bg-elevated p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-md hover:border-brand-500/40 transition-all">
                  <div className="min-w-0">
                    <p className="font-extrabold text-theme-heading text-sm sm:text-base">{m.title}</p>
                    <p className="text-xs font-semibold text-theme-body mt-1">
                      {new Date(m.date).toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} · {m.time} · {m.duration} min · Host: <span className="font-bold text-brand-600 dark:text-brand-400">{m.host}</span>
                    </p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-bold text-theme-body hover:text-brand-600 rounded-full border border-soft px-3 py-1.5 hover:bg-surface transition-colors cursor-pointer"
                    >
                      <Copy size={13} />
                    ...
                    <button
                      ...
                      className="flex h-8 w-8 items-center justify-center rounded-full border border-soft text-muted hover:text-rose-400 hover:border-rose-400/40 transition-colors cursor-pointer"
                    >
                      <Trash2 size={14} />
                    </button>
          ...
          {meetingHistory.length === 0 ? (
            ...
            <ul className="flex flex-col gap-3">
              {meetingHistory.map((h) => (
                <li key={h.id} className="rounded-2xl border border-soft bg-elevated p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-md hover:border-brand-500/40 transition-all">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="font-extrabold text-theme-heading text-sm sm:text-base">{h.title}</p>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      className="text-xs font-extrabold rounded-full bg-brand-500/15 hover:bg-brand-500 text-brand-600 hover:text-ink-950 border border-brand-500/30 px-4 py-1.5 transition-all shadow-sm cursor-pointer"
                    >
                      Rejoin
                    </button>
                  </div>
